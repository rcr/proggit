...
int
rank(int ranks[7], int suits[7])
{
	int i, j, straight, flush = 0, suit_count[5] = {0};
...
#define CHOOSE(a,b) ranks[a] = ranks_i[b]; suits[a] = suits_i[b];

int
rank_i(char *hand)
{
	/* Read */
	int i, j, k, l, m, temp, ranks_i[9], suits_i[9];
	for (i = 0; i < 9; i++){
		ranks_i[i] = get_rank(hand[i*3]);
		suits_i[i] = get_suit(hand[i*3 + 1]);
	}

	/* Iterate 60 Combinations, (5 choose 3)*(4 choose 2) */
	/* This is an abomination, avert your eyes */
	int best = 0, ranks[7] = {0}, suits[7] = {0};
	for (i = 0; i < 3; i++){
		for (j = i+1; j < 4; j++){
			for (k = j+1; k < 5; k++){
				for (l = 5; l < 8; l++){
					for(m = l+1; m < 9; m++){
						...
						CHOOSE(3,l);
						CHOOSE(4,m);
						best = (temp = rank(ranks, suits)) > best ? temp : best;
					}
				}
			}
		}
...
main(int argc, char **argv)
{
	...
	for (fscanf(fp, "%d", &rounds); rounds > 0; rounds--){
		...
		for (winner_i = high = i = 0; i < hands; i++){
			...
			fread(&hand[15], sizeof(char), 12, fp);

			if ((temp = rank_i(hand)) > high){
				winner[0] = i;
				winner_i = 0;
				high = temp;
			} else if (temp == high)
